...
namespace UnityGameFramework.Editor.AssetBundleTools
{
    internal sealed partial class AssetBundleAnalyzerController
    {
        ...
        {
            m_AssetBundleCollection = (assetBundleCollection ?? new AssetBundleCollection());
        ...
        public string[] GetAssetNames(AssetsOrder order, string filter)
        {
            ...
            IEnumerable<KeyValuePair<string, DependencyData>> filteredResult = m_DependencyDatas.Where(pair => 
                filteredAssetNames.Contains(pair.Key));
            IEnumerable<KeyValuePair<string, DependencyData>> orderResult = null;
            switch (order)
            {
                case AssetsOrder.AssetNameAsc:
                    orderResult = filteredResult.OrderBy(pair => pair.Key);
                    break;
                case AssetsOrder.AssetNameDesc:
                    orderResult = filteredResult.OrderByDescending(pair => pair.Key);
                    break;
                case AssetsOrder.DependencyAssetBundleCountAsc:
                    orderResult = filteredResult.OrderBy(pair => pair.Value.DependencyAssetBundleCount);
                    break;
                case AssetsOrder.DependencyAssetBundleCountDesc:
                    orderResult = filteredResult.OrderByDescending(pair => pair.Value.DependencyAssetBundleCount);
                    break;
                case AssetsOrder.DependencyAssetCountAsc:
                    orderResult = filteredResult.OrderBy(pair => pair.Value.DependencyAssetCount);
                    break;
                case AssetsOrder.DependencyAssetCountDesc:
                    orderResult = filteredResult.OrderByDescending(pair => pair.Value.DependencyAssetCount);
                    break;
                case AssetsOrder.ScatteredDependencyAssetCountAsc:
                    orderResult = filteredResult.OrderBy(pair => pair.Value.ScatteredDependencyAssetCount);
                    break;
                case AssetsOrder.ScatteredDependencyAssetCountDesc:
                    orderResult = filteredResult.OrderByDescending(pair => pair.Value.ScatteredDependencyAssetCount);
                    break;
                default:
                    orderResult = filteredResult;
                    break;
            }

            return orderResult.Select(pair => pair.Key).ToArray();
        }
        ...
        public string[] GetScatteredAssetNames(ScatteredAssetsOrder order, string filter)
        {
            ...
            IEnumerable<KeyValuePair<string, List<Asset>>> filterResult = m_ScatteredAssets.Where(pair
                => filterAssetNames.Contains(pair.Key) && pair.Value.Count > 1);
            IEnumerable<KeyValuePair<string, List<Asset>>> orderResult = null;
            switch (order)
            {
                case ScatteredAssetsOrder.AssetNameAsc:
                    orderResult = filterResult.OrderBy(pair => pair.Key);
                    break;
                case ScatteredAssetsOrder.AssetNameDesc:
                    orderResult = filterResult.OrderByDescending(pair => pair.Key);
                    break;
                case ScatteredAssetsOrder.HostAssetCountAsc:
                    orderResult = filterResult.OrderBy(pair => pair.Value.Count);
                    break;
                case ScatteredAssetsOrder.HostAssetCountDesc:
                    orderResult = filterResult.OrderByDescending(pair => pair.Value.Count);
                    break;
            }
        ...
        {
            string[] filteredAssetGuids = AssetDatabase.FindAssets(filter);
            HashSet<string> filterAssetNames = new HashSet<string>();
            foreach (string filterAssetGuid in filteredAssetGuids)
            {
