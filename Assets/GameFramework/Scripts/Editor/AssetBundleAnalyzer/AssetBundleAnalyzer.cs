...
namespace UnityGameFramework.Editor.AssetBundleTools
{
    ...
    internal sealed class AssetBundleAnalyzer : EditorWindow
    {
        ...
        private Vector2 m_AssetScroll = Vector2.zero;
        ...
        private void OnGUI()
        {
            ...
            {
                GUILayout.Space(5f);
                int toolbarIndex = GUILayout.Toolbar(m_ToolbarIndex, new string[]
                    { "Summary", "Asset Dependency Viewer", "Scattered Asset Viewer"}, GUILayout.Height(30f));
        ...
        private void DrawAnalyzeButton()
        {
            if (!m_Analyzed)
            {
                EditorGUILayout.HelpBox("Please analyze first.", MessageType.Info);
                {
                    ...
                    {
                        EditorUtility.DisplayDialog("AssetBundle Analyze", "Can not parse 'AssetBundleCollection.xml', " +
                            "please use 'AssetBundle Editor' tool first.", "OK");
                    }
                }
            }
        }
        ...
        private void DrawAssetDependencyViewer()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.4f));
                {
                    ...
                    {
                        m_AssetScroll = EditorGUILayout.BeginScrollView(m_AssetScroll);
                        {
        ...
        {
            EditorUtility.DisplayProgressBar("Loading AssetBundles", string.Format("Loading AssetBundles, {0}/{1} loaded.",
                index.ToString(), count.ToString()), (float)index / count);
        }

        private void OnLoadingAsset(int index, int count)
        {
            EditorUtility.DisplayProgressBar("Loading Assets", string.Format("Loading Assets, {0}/{1} loaded.",
                index.ToString(), count.ToString()), (float)index / count);
        }
        ...
        {
            EditorUtility.DisplayProgressBar("Analyzing Assets", string.Format("Analyzing Assets, {0}/{1} analyzed.",
                index.ToString(), count.ToString()), (float)index / count);
        }
