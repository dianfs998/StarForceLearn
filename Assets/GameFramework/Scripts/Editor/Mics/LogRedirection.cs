...
namespace UnityGameFramework.Editor
{
    ...
    internal static class LogRedirection
    {
        ...
        private static bool OnOpenAsset(int instanceId, int line)
        {
            ...
            {
                //直接使用GameFramework.dll码源而非dll的工程会多一次匹配的堆栈
                match = match.NextMatch();
                if (!match.Success)
                {
                    return false;
            ...
            InternalEditorUtility.OpenFileAtLineExternal(Utility.Path.GetCombinePath(Application.dataPath, 
                match.Groups[1].Value.Substring(7)), int.Parse(match.Groups[2].Value));
            return true;
        }
        ...
        private static string GetSelectedStackTrace()
        {
            ...
            FieldInfo activeTextFieldInfo = consoleWindowType.GetField("m_ActiveText",
